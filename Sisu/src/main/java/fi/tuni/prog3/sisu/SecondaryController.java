 ...
 * This is an additional part of the project.
 *
 * Class offers public and private getter and setter functions. Also offers
 * FXML-switch functions for functionality. Controlled FXML asks for following
 * user input:
 * <ul>
...
public class SecondaryController implements Initializable {
    ...
    /*
    * Sets the view to english version of primary.fxml
     */
    @FXML
    ...
    public int addTostudentData(String name,
        ...
        if (Integer.valueOf(start) >= Integer.valueOf(finish)) {
            System.out.println("inputerrorEN");
            errors = true;
        }
        ...
        if (name.matches("[0-9]+") || number.matches("[a-zA-Z]+") || start
            .matches("[a-zA-Z]+") || finish.matches("[a-zA-Z]+") || start
            .length() != 4 || finish.length() != 4) {
            System.out.println("inputerrorEN");
            errors = true;
        }
    ...
    * Then  sets view to mainPage.
     */
    private void switchToMainPageEN() throws IOException {

        int errors = addTostudentData(name.getText(), number.getText(),
            start.getValue().toString(), finish.getValue().toString());

        if (errors == 0) {
            addToJson(name.getText(), number.getText(),
                start.getValue().toString(), finish.getValue().toString());
            Sisu.setRoot("mainPageEN");
        } else if (errors == 1) {
            Sisu.setRoot("inputerrorEN");
        }
    }
